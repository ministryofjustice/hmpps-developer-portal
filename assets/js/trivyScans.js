jQuery(function () {
  const columns = [
    {
      ...
      createdCell: function (td, _cellData, rowData) {
        $(td).html(`${rowData.attributes.name}`)
      },
    },
    {
      data: 'attributes.build_image_tag',
      createdCell: function (td, _cellData, rowData) {
        $(td).html(`${rowData.attributes.build_image_tag}`)
      },
    },
    {
      data: 'attributes.trivy_scan_results',
      createdCell: function (td, _cellData, rowData) {
        $(td).html(`${rowData.attributes.trivy_scan_results}`)
      },
    },
    {
      data: 'attributes.trivy_scan_timestamp',
      createdCell: function (td, _cellData, rowData) {
        $(td).html(`${rowData.attributes.trivy_scan_timestamp}`)
      },
    },
    // {
  ...
  createTable({ id: 'trivyScansTable', ajaxUrl: '/trivy-scans/data', orderColumn: 0, orderType: 'asc', columns })
})
